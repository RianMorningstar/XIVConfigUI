...
/// </summary>
public class DrawingHighlightHotbar : BaseDraw
{
    ...
    protected unsafe override void DrawInside()
    {
        if (_texture == null) return;
                ...
                foreach (var slot in actionBar->ActionBarSlotVector.AsSpan())
                {
                    ...
                    if ((nint)iconAddon != nint.Zero && IsVisible(&iconAddon->AtkResNode))
                    {
                        ...
                        if (IsActionSlotRight(slot, bar))
                        {
                            ...
                            ImGui.GetWindowDrawList().AddImage(_texture.ImGuiHandle, pt1, pt2, _uv1, _uv2, ImGui.ColorConvertFloat4ToU32(Color));
                        }
                    }
